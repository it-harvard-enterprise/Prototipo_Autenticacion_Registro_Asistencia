...
func UpdateStudentFingerprintsHandler(app *services.App) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req models.UpdateStudentFingerprintsRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid JSON payload"})
			return
		}
		...
		if numeroIdentificacion == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "numero_identificacion es requerido"})
			return
		}

		hasRightPayload := req.HuellaIndiceDerechoEncrypted != nil ||
			(req.HuellaIndiceDerecho != nil && strings.TrimSpace(*req.HuellaIndiceDerecho) != "")
		hasLeftPayload := req.HuellaIndiceIzquierdoEncrypted != nil ||
			(req.HuellaIndiceIzquierdo != nil && strings.TrimSpace(*req.HuellaIndiceIzquierdo) != "")

		if !hasRightPayload && !hasLeftPayload {
			c.JSON(http.StatusOK, gin.H{"success": true, "message": "No fingerprint changes received"})
			return
		}
		...
		if hasRightPayload {
			var rightPNG *string
			if req.HuellaIndiceDerechoEncrypted != nil {
				decrypted, err := app.DecryptPNG(req.HuellaIndiceDerechoEncrypted)
				if err != nil {
					c.JSON(http.StatusBadRequest, gin.H{"error": "Huella derecha no se pudo desencriptar: " + err.Error()})
					return
				}
			...
			if rightTemplate == services.DefaultFingerprint {
				payload["huella_indice_derecho"] = nil
			} else {
				encPayload, err := app.EncryptTemplate(rightTemplate)
				if err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": "Error encriptando huella derecha: " + err.Error()})
				...
				payload["huella_indice_derecho"] = encPayload
			}
		}

		if hasLeftPayload {
			var leftPNG *string
			if req.HuellaIndiceIzquierdoEncrypted != nil {
				decrypted, err := app.DecryptPNG(req.HuellaIndiceIzquierdoEncrypted)
				if err != nil {
					c.JSON(http.StatusBadRequest, gin.H{"error": "Huella izquierda no se pudo desencriptar: " + err.Error()})
					return
				}
			...
			if leftTemplate == services.DefaultFingerprint {
				payload["huella_indice_izquierdo"] = nil
			} else {
				encPayload, err := app.EncryptTemplate(leftTemplate)
				if err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": "Error encriptando huella izquierda: " + err.Error()})
				...
				payload["huella_indice_izquierdo"] = encPayload
			}
		}

		rows, status, err := app.UpdateStudentByNumero(c.Request.Context(), numeroIdentificacion, payload)
		if err != nil {
		...
		if len(rows) == 0 {
			c.JSON(http.StatusNotFound, gin.H{"error": "No se encontro el estudiante para actualizar huellas"})
			return
		}

		c.JSON(http.StatusOK, gin.H{"success": true, "data": rows[0]})
	}
}
...
func IdentifyPersonByFingerprintHandler(app *services.App) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req models.PersonIdentifyByFingerprintRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "Invalid JSON payload"})
			return
		}
		...
		students, err := app.ListStudentsWithTemplates(c.Request.Context())
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
		...
		for _, student := range students {
			templatesToTry := []*string{student.HuellaIndiceDerecho, student.HuellaIndiceIzquierdo}
			for _, encryptedTemplateStr := range templatesToTry {
				if encryptedTemplateStr == nil || strings.TrimSpace(*encryptedTemplateStr) == "" {
					continue
				}
				...
				if err := json.Unmarshal([]byte(*encryptedTemplateStr), &encPayload); err != nil {
					...
					score := matcher.Match(ctx, candidate)
					if bestID == "" || score > bestScore {
						bestID = student.NumeroIdentificacion
						bestScore = score
					}
					continue
				}
