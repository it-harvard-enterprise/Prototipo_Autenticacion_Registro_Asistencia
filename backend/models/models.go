...
type UpdateStudentFingerprintsRequest struct {
	NumeroIdentificacion string `json:"numero_identificacion"`

	// Plain PNG base64 payloads (legacy compatibility)
	HuellaIndiceDerecho   *string `json:"huella_indice_derecho"`
	HuellaIndiceIzquierdo *string `json:"huella_indice_izquierdo"`

	// AES-GCM encrypted PNG payloads (preferred)
	HuellaIndiceDerechoEncrypted   *EncryptedPayload `json:"huella_indice_derecho_encrypted"`
	HuellaIndiceIzquierdoEncrypted *EncryptedPayload `json:"huella_indice_izquierdo_encrypted"`
}
...
type PersonIdentifyByFingerprintRequest struct {
	FingerprintTemplate string `json:"fingerprint_template"`
}
